...
'ftpuse e: ftpbackup3.jnb1.host-h.net H3tz_FtP /user:pri004_RE4
...
CONST conCopyToFolder = "E:"
CONST conPurgeFolder = "E:\Purge"
...
fileName = "BackupWebsitesToFTP-" & strGetDateAsNumerics(Now) & ".log"
...
If WScript.Arguments.Count <> 1 Then 
	If conDebugMode Then
		WScript.Echo "No parameter received. Quitting."
	End If
...
If Not conDebugMode Then
  If objFSO.DriveExists("e:") = False Then
    On Error Resume Next        
    objAppShell.ShellExecute "ftpuse.exe", "e: ftpbackup3.jnb1.host-h.net H3tz_FtP /user:pri004_RE4", "", "", 7
...
If Not conDebugMode Then
  If objFSO.DriveExists("e:") = True Then
    On Error Resume Next        
    objAppShell.ShellExecute "ftpuse.exe", "e: /delete", "", "", 7
    On Error Goto 0  
  End If
End If		
...
Sub DeleteOldBackups
	...
	For Each SubFolder In objSubFolders
		...
		If Err.Number = 0 Then		
			If conDebugMode Then Wscript.Echo "DeleteOldBackups: SubFolder.Name as date = " & dteFolderDate
			...
			If dteFolderDate < dteDate Or SubFolder.Name = strTestDate Then
				...
				Else
					wscript.echo "DeleteOldBackups: Deleting " & conCopyToFolder & "\" & SubFolder.Name
				End If		
			End If
		End If
	Next	
...
Sub StartZip
	...
	If Not objFSO.FolderExists(conCopyToFolder & "\" & strFolderDate) Then
		If Not conDebugMode Then
			...
			On Error Goto 0
		Else
			wscript.echo "StartZip: Creating folder " & conCopyToFolder & "\" & strFolderDate
		End If
	End If
	...
	For Each SubFolder In objSubFolders
		...
		If Not conDebugMode Then							
			...
			End If
		Else		
			wscript.echo "StartZip: Creating folder " & conCopyToFolder & "\" & strFolderDate & "\" & SubFolder.Name
		End If
		...
		Else		
			...
			wscript.echo "StartZip: Moving file " & strZipFile & vbcrlf & "to " & conCopyToFolder & "\" & strFolderDate & "\" & SubFolder.Name & vbcrlf & strCmd
		End If
...
Function DoZip(strFolderPathAndNameToZip, strZipFilePathAndName)			    
    If Not objFSO.FolderExists(strFolderPathAndNameToZip) Then
		If conDebugMode Then			
			WScript.Echo "DoZip: Folder " & strFolderPathAndNameToZip & " to zip does not exist."
		Else
	...
	If objFSO.FileExists(strZipFilePathAndName) Then
		If conDebugMode Then
			WScript.Echo "DoZip: File " & strZipFilePathAndName & " already exists - deleting it."
		Else
	...
	If conDebugMode Then
		wscript.echo "DoZip: Zip cmd = " & strCmd
	Else
...
Sub CopyAdHocFiles
	...
	If Not conDebugMode Then
		...
		'	wscript.echo conCopyToFolder & "\" & strFolderDate & "\AdHoc\MiscInfo"
