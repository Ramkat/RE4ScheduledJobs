		  ...
		  If Len(Trim(arrData(5, intLoop))) > 0 Then 	
				...
				If LCase(arrData(3, intLoop)) <> "dummy@dummy.dummy" Then
					...
					Else
						...
						If blnDebug Then 
							wscript.echo "Mailhost not found - " & arrData(5, intLoop)
							wscript.echo strResult7
						End If
					End If
				Else
...
Sub GetEmails
	...
	If blnDebug Then Wscript.echo intEmails & " to process"
...
Function UpdateResults
	...
	If blnDebug Then wscript.echo strSQL
...
Function OpenADOConnection
	...
	'ODBC Connection
	strConnString = "driver={SQL Server};pwd=~letzgetem@1l!;uid=EmailUser;database=RE4-Products;Server=127.0.0.1,11523"
...
Exit Function
  ...
  If Err.number <> 0 Then  
	...
	If blnDebug Then wscript.echo "Email host = " & strHost
  ...
  'Check the result.
	...
	Else			
		...
		If blnDebug Then wscript.echo "MxLookup result = " & strResult
  
		If CInt(strResult) > 0 Then
		  MXLookup = True	'Host found!
		  
		  If blnDebug Then wscript.echo "Mailhost found"
		End If
	End If 
