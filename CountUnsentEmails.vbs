...
Sub TestSQL
	...
	'This sub will count all emails.	
	strSQL = "select count(*) as Counted from email where emailstatusid = 1; select min(dateadded) as MinDate from email where emailstatusid = 1"
	...
	If ADORs.RecordCount > 0 And ADORs("Counted") > 0 Then
		...
		If ADORs.RecordCount > 0 Then
			strMsg = strMsg & vbcrlf & "Oldest unsent email: " & ADORs("MinDate")
		End If
	Else
		strMsg = "No unsent records found in the Email table."
	End If		
