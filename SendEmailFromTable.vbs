...
'		     : Added MS365 SMTP server logic(again) - 26 June 2023
...
'###
Call LogSendError2File("0", "Script started @ " & Now())

Call Main

Call LogSendError2File("0", "Script ended @ " & Now())
Call LogSendError2File("0", "= = = = = = = = = = = = = = = =")
...
Sub Main
	...
	'###
	'Call LogSendError2File("0", "Inside sub Main()")
	...
	'###
	'Call LogSendError2File("0", "Past CDO creation. Type = " & TypeName(objSendMail) )
		
	If Err.Number <> 0 Then
		If blnDebug Then 
			Output "No CDO email object created." & vbcrlf & Err.Description
			'Call LogSendError2File("0", "No CDO email object created.")
		End If
	...
	'Set a few SMTP server settings
	...
	objSendMail.Configuration.Fields.Item ("http://schemas.microsoft.com/cdo/configuration/smtpserver") = "196.22.138.229" '"154.66.66.123"
	...
	'###
	'Call LogSendError2File("0", "Past objSendMail configuration")
	
	'###
	'Call LogSendError2File("0", "Calling GetEmails()")
	...
	Do While blnGetEmails									
		'###
		'Call LogSendError2File("0", "objSendMail type = " & TypeName(objSendMail) )
		
		'###
		'Call LogSendError2File("0", "MS365Mailer type = " & TypeName(objMS365Mailer))
		  ...
		  If Len(Trim(arrData(5, intLoop))) > 0 Then 	
				...
				If LCase(arrData(3, intLoop)) <> "dummy@dummy.dummy" Then
					...
					If MXLookup(arrData(5, intLoop)) Then
						...
						'If the email is sent from info@wildlifecampus.com, use the Miscrosoft 365 SMTP server.
						'If arrData(3, intLoop) = "info@wildlifecampus.com" Then
						'	
						'	Call SendWithMS365(intLoop)
						'	
						'Else 'Use our own (IIS) SMTP server
							'Set the FromName and FromAddress
							strFrom = chr(34) & arrData(2, intLoop) & chr(34) & " " & arrData(3, intLoop)
							objSendMail.From = strFrom
					
							'Set the recipients' ToName and ToAddress
							...
							If Err.number <> 0 Then
								...
								Call LogSendError2File(arrData(0, intLoop), strFrom & " to " & strTo & vbcrlf & Err.Description)
							Else
...
Sub SendWithMS365(intLoop)
	Dim objMS365Mailer
	
	On Error Resume Next
	Err.Clear
	Set objMS365Mailer = CreateObject("RE4.MS365Mailer")
	
	'###
	Call LogSendError2File("0", "Past MS365Mailer creation. Type = " & TypeName(objMS365Mailer) & ". Err.Number = " & Err.Number)
	
	If Err.Number <> 0 Then
		If blnDebug Then 
			Output "No MS365Mailer object created." & vbcrlf & Err.Description
			Call LogSendError2File("0", "MS365Mailer object creation failed: " & Err.Description)
		End If
		
		Set objMS365Mailer = Nothing					
		Exit Sub
	End If
							
	'SendEmail(string from, string fromName, string replyTo, string to, string cc, string bcc, string subject, string body, string smtpServer, int port, bool useSSL, bool isHTML, string username, string password, bool debug, string logFile = "")
	strErrMsg = objMS365Mailer.SendEmail(arrData(3, intLoop), arrData(2, intLoop), arrData(3, intLoop), arrData(5, intLoop), "", "", arrData(1, intLoop), arrData(6, intLoop), "smtp.office365.com", 587, True, False, "info@wildlifecampus.com", "!JuZtM@iL!", False, "")
	...
	ElseIf strErrMsg <> "" Then
		strResult9 = strResult9 & "," & arrData(0, intLoop)
		Call LogSendError2File(arrData(0, intLoop), strErrMsg)
	Else
		strResult1 = strResult1 & "," & arrData(0, intLoop)
	End If
...
Sub GetEmails
	...
	'###
	Call LogSendError2File("0", intEmails & " emails to process")
...
Sub LogSendError2File(strEmailID, strErrDesc)
	...
	Set objFile = objFSO.OpenTextFile("D:\ScheduledJobs\SendEmailFromTable.log.txt", 8, True)
...
Sub LogError2File(strErrDesc)
	...
	Set objFSO = CreateObject("Scripting.FileSystemObject")	
	Set objFile = objFSO.OpenTextFile("D:\ScheduledJobs\SendEmailFromTable_SQLErrors.txt", 8, True)
